...
func (s *signerServer) SignMessage(_ context.Context,
	...
	if in.SchnorrSig {
		sig, err := s.wallet.SignMessageSchnorr(
	...
	if in.CompactSig {
		sigBytes, err := s.wallet.SignMessageCompact(
			keyLocator, in.Msg, in.DoubleHash,
		)
		if err != nil {
			return nil, fmt.Errorf("can't sign the hash: %v", err)
...
func (s *signerServer) DeriveSharedKey(_ context.Context,
	...
	sharedKeyHash, err := s.wallet.ECDH(keyDescriptor, ephemeralPubkey)
	if err != nil {
...
func (s *signerServer) MuSig2CombineKeys(_ context.Context,
	in *proto.MuSig2CombineKeysRequest) (
	*proto.MuSig2CombineKeysResponse, error) {
	...
	return &proto.MuSig2CombineKeysResponse{
...
func (s *signerServer) MuSig2CreateSession(_ context.Context,
	...
	session, err := s.wallet.Cfg.Signer.MuSig2CreateSession(
...
func (s *signerServer) MuSig2RegisterNonces(_ context.Context,
	...
	*proto.MuSig2RegisterNoncesResponse, error) {
	...
	haveAllNonces, err := s.wallet.Cfg.Signer.MuSig2RegisterNonces(
...
func (s *signerServer) MuSig2Sign(_ context.Context,
	...
	partialSig, err := s.wallet.Cfg.Signer.MuSig2Sign(sessionID, msg, in.Cleanup)
	if err != nil {
...
func (s *signerServer) MuSig2CombineSig(_ context.Context,
	in *proto.MuSig2CombineSigRequest) (
	*proto.MuSig2CombineSigResponse, error) {
	...
	finalSig, haveAllSigs, err := s.wallet.Cfg.Signer.MuSig2CombineSig(
	...
	resp := &proto.MuSig2CombineSigResponse{
...
func (s *signerServer) MuSig2Cleanup(_ context.Context,
	in *proto.MuSig2CleanupRequest) (*proto.MuSig2CleanupResponse, error) {
	...
	err = s.wallet.Cfg.Signer.MuSig2Cleanup(sessionID)
	if err != nil {
		return nil, fmt.Errorf("error cleaning up session: %v", err)
	}

	return &proto.MuSig2CleanupResponse{}, nil
}
...
// counterpart.
func UnmarshalTweaks(rpcTweaks []*proto.TweakDesc,
	taprootTweak *proto.TaprootTweakDesc) (*input.MuSig2Tweaks, error) {
